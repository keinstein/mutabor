...
Comments\index{comment} an entailed in douple quotes
(\texttt{\dq})\index{*''@\dq (double quote)}.  Comments cannot be
nested, but can span multiple lines.
...
(\textbf{'}) \index{*'@\textapostrophe\ (apostrophe)}.\footnote {The underscore is one of the letters, so
you can write meaningful names legibly, e.\,g.,
\texttt{LOGIC third\_tone\_mutating} is better than
\texttt{LOGIC thirdtonemutating},
and the apostrophe counts as a letter
so that tone with a simple or multiple apostrophies can
provide the name, e.\,g \texttt{TONE e${}'''$}}
...
In the books of \mutabor{} the terms \textit{name} \index{name}
and \textit{identifier}\index{identifier} are
used in parallel and with equal meaning. Furthermore, in some
syntax graphs also terms of the type \textit{interval name}, \textit{tone
  name}, \ldots are
...
comes into question.
\iffalse
In den Büchern von \mutabor{} werden die Begriffe \textit{Name}\index{Namen}
und \textit{Bezeichner}\index{Bezeichner}
parallel und mit gleicher Bedeutung verwendet. Ferner werden bei einigen
Syntaxgraphen auch Begriffe der Art \textit{Intervallname}, \textit{Tonname},\ldots
...
\iffalse
Ähnlich wie bei Namen werden Zahlen im Syntaxdiagramm auch als \textit{Faktor}
oder \textit{Verankerungs-Taste} bezeichnet,
...
\textit{in eckigen Kästchen} und reservierte Worte und Zeichen \textit{in
...
\texttt{A WURZEL B} ist die \keyword{A}te Wurzel aus \keyword{B}.
...
Symbol \textbf{@}\index{at@\textat\ (at)} `\textit{der alte Wert}',
...
\iffalse
  \item[Umstimmung-Töne]\index{Umstimmung!Töne}
      ...
      zu schreiben, weil
      `\textit{nichts}' bedeutet, daß der Ton stumm geschaltet wird.
...
\textit{globale Auslöser}\index{Auslöser!globale}
 und letztere \textit{lokale Auslöser}\index{Auslöser!lokale}. Beim Eintreffen
...
Dieser Auslöser\index{Auslöser!Taste}
 besteht aus dem Wort \textit{Taste}\index{Tasten}, gefolgt von einem der
...
sich selbst widerspricht. Solche Harmonien sind '\textit{unmögliche}'
...
\keyword{ELSE}\index{ELSE}. This branch will be executed if the harmony analysis
...
\iffalse % the following is not true
...
\iffalse
MIDI-Ereignisse werden mit dem Wort \textit{MIDIIN}\index{MIDIIN}
eingeleitet und
sind eine eingeklammerte Folge von Zahlen. Bei Zahlen ist die sedezimale
Schreibweise zulässig (\#\textit{ziffern}).\index{*#@\#}
...
Die Status-Bytes\index{Status-Byte}
 \textit{Note-On\index{Note-On} (\#90)}, \textit{Note-Off\index{Note-Off}
  (\#80)} und
\textit{System-Byte\index{System-Byte}
 (\#F0 - \#FF)} werden nicht in die MIDI-Analyse
einbezogen.
\fi

MIDI bytes are compared messagewise. This leads to the restriction,
...
sowie der Wert \textit{ABSTAND}\index{ABSTAND}.
...
\emph{HARMONY\_ANALYSIS}\index{HARMONY\_ANALYSIS}. Such a harmony
...
MIDI-Ausgaben werden mit dem Wort \textit{MIDIOUT}\index{MIDIOUT}
 eingeleitet und
sind eine eingeklammerte Folge von Zahlen. Bei Zahlen ist die sedezimale
Schreibweise zulässig (\#\textit{ziffern}). Die Zahlen
müssen im Bereich von 0 bis 255 liegen, also Bytes sein. Es erfolgt keine
Kontrolle, ob es sich um eine sinnvolle MIDI-Nachricht handelt.
\fi
...
Die MIDI-In-Kanäle werden auch als \textit{Instrumente} bezeichnet, da
