...
the underscore (\_) \index {\_ (underscore)} and the apostrophe
({\bf'}) \index {' (apostrophe)}.\footnote {The underscore is one of the letters, so
...
\ei
\fi
Here \texttt{Number1 : Number2} denotes the ratio
\begin{align*}
\frac{\text{\texttt{Number1}}}{\text{\texttt{Number2}}},
\end{align*}
and \texttt{A ROOT B} is  the \texttt
Ath root of \texttt B:
\begin{align*}
  \sqrt[\text{\texttt{A}}]{\text{\texttt B}}.
\end{align*}
...
\begin{align*}
  \mathtt I = \texttt{octave}^3 * \texttt{third}^{-5} * \texttt{cent}^{17}.
\end{align*}
\iffalse
Es bedeutet dabei
...
\[
\text{frequency}(\texttt{b}) = \text{frequency}(\texttt{c}) *
\texttt{fifth}^{-2} * \texttt{octave}.
\]
\iffalse
...
symbol \textbf{\@}\index{\@ (at)} means semantically “\textit{the old
...
Symbol {\bf @}\index{\@ (at)} `{\it der alte Wert}',
...
\fi
    ...
    \item[Retuning tones]\index{retuning!tones} corresponds to a
      ...
      \texttt{\@}\index{{\@ (at)}} must be used, sine \emph{nothing}
...
\iffalse
  \item[Umstimmung-Töne]\index{Umstimmung!Töne}
      ...
      erhalten bleiben, so ist dafür das Symbol \texttt{\@}\index{{\@ (at)}}
      zu schreiben, weil
...
\fi
    ...
    \item[Retuning alternative]\index{retuning!alternative} A retuning
      ...
      delimited by commas (\texttt{,}), while complete alternatives
      are not seperated by any special delimiter. The constants of the
      alternatives must be unique integers. At the end of an
      alternative a special constant \texttt{else}\index{else} can be
      used. If the selector coinsides with a constant the
      corresponding alternative is executed. If it is not equals to
      any constant and the constant \texttt{else} is given the
      alternative of the \texttt{else} branch is used. If the selector
      is different from any constant and the special constant \texttt{else} is
...
\fi
    ...
    \item[Action MIDI out]\index{action!MIDI out} a MIDI output is
      started with the word \texttt{MIDIOUT}\index{MIDIOUT} and is a
