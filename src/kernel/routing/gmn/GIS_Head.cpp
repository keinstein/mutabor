 ...
 * Description
 *
 * $Header: /home/tobias/macbookbackup/Entwicklung/mutabor/cvs-backup/mutabor/mutabor/src/kernel/routing/gmn/GIS_Head.cpp,v 1.7 2011/09/30 18:07:04 keinstein Exp $
...
mutint64 GetTheSpeedFactor(GisToken *token)
{
	...
	if ( token && GetGisType(token) == GTParaStr ) {
		...
		mutint64 numerator = 0, denominator = 0, bpm=0;
	...
	if ( *(position->PrevPtr) == position ) { // first position
		...
		*(position->PrevPtr) = this;
		...
		DEBUGLOG (gmnfile, _T("first position %p, Prev: %p, Next: %p, cmp: %p"),
		          ...
		          (void*)position->Prev,
		          (void*)position->Next,
		          (void*)*((GisReadHead**)(position->PrevPtr))
		        );
