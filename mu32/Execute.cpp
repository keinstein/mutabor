...
TONSYSTEM tonsystem_memory[MAX_BOX+1];  /* Der Platz fÅr die tatsÑchlichen
												tonsysteme */

TONSYSTEM tonsystem_init =
      {60, 1, DOUBLE_TO_LONG(1), { DOUBLE_TO_LONG(60) }} ;

TONSYSTEM *tonsystem[MAX_BOX]       /* Verweise auf die aktuellen
...
/*	  = { tonsystem_memory + 0,
         ...
         tonsystem_memory + 15 }*/;
TONSYSTEM * freies_tonsystem    /* Freizeiger auf das */
...
void update_pattern(int instr)
{
  ...
  TONSYSTEM * tonsys = tonsystem[instr];
	 int i;
...
void HarmonyAnalysis(int box, PATTERNN * pattern )
{
  ...
  TONSYSTEM *tonsys = tonsystem[box];
  int i;
